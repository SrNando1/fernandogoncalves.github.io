        ...
        if ($result->num_rows === 1) {
            ...
            if (password_verify($password, $user['senha'])) {
                ...
                if ($user['is_admin'] == 1) {
                    ...
                    header("Location: dashboard-admin.php"); // Redirecionar para a página do administrador
                    exit();
                } else {
                    // Se o usuário não for admin, consideramos como erro (mesmo comportamento de login incorreto)
                    header("Location: loginFormAdmin.php?error=" . urlencode("Senha ou Usuário incorretos!"));
                    exit();
                }
            } else {
                // Senha incorreta (comportamento igual a erro)
                header("Location: loginFormAdmin.php?error=" . urlencode("Senha ou Usuário incorretos!"));
                exit();
            }
        } else {
            // Usuário não encontrado (comportamento igual a erro)
            header("Location: loginFormAdmin.php?error=" . urlencode("Senha ou Usuário incorretos!"));
            exit();
        }
    }
